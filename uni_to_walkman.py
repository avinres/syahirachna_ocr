CHARS_UNICODE = [#// ignore all nuktas except in ड़ and ढ़
        "[", "]", "‘", "’", "“", "”", "%", "{", "}", "/", ":", "=", "।", "?", "-", "µ", "॰", ",", ".", "् ",
...
def uni_to_walkman(uni_text):
    ...
    if uni_text != "":
        ...
        uni_text = uni_text.replace("क़", "क़");
        uni_text = uni_text.replace("ख़‌", "ख़");
        uni_text = uni_text.replace("ग़", "ग़");
        uni_text = uni_text.replace("ज़", "ज़");
        uni_text = uni_text.replace("ड़", "ड़");
        uni_text = uni_text.replace("ढ़", "ढ़");
        uni_text = uni_text.replace("ऩ", "ऩ");
        uni_text = uni_text.replace("फ़", "फ़");
        uni_text = uni_text.replace("य़", "य़");
        uni_text = uni_text.replace("ऱ", "ऱ");
        ###############################################################
        #replacing #  ? + ि  ->  f + ?
        ...
        for input_symbol_idx in range (len_uni):
            ...
            while(idx != -1):
                ...
                b = CHARS_WALK[input_symbol_idx]
                ##print(a, b)
                uni_text = uni_text.replace(a, b,1)
                idx = uni_text.find(CHARS_UNICODE[input_symbol_idx])
                ##print(idx)
    uni_text = uni_text.replace("Zksa", "ks±");
    uni_text = uni_text.replace("~ Z", "Z~");
    uni_text = uni_text.replace("Zk", "kZ");
    uni_text = uni_text.replace("Zh", "hZ");
    uni_text = uni_text.replace("aZ", "±");
    return uni_text

def convert_to_walkman(uni_string):
    walk_string = ''
    ...
    while chale_chalo == 1:
        ...
        walk_string += uni_to_walkman(uni_text)
    return walk_string
